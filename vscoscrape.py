...
visituserinfo = {
    ...
    "Connection": "keep-alive",
    "Host": "vsco.co",
    "Referer": "https://vsco.co/bob/images/1",
    "User-Agent": random.choice(user_agents),
}
...
class Scraper(object):
    ...
    def newSiteId(self):
        ...
        res = self.session.get(
            "https://vsco.co/ajxp/%s/2.0/sites?subdomain=%s" % (self.uid, self.username)
        )
    ...
    def buildJSON(self):
        ...
        """
        self.mediaurl = "https://vsco.co/ajxp/%s/2.0/medias?site_id=%s" % (
            self.uid,
            self.siteid,
        )
        self.journalurl = "https://vsco.co/ajxp/%s/2.0/articles?site_id=%s" % (
            self.uid,
            self.siteid,
        )
        self.collectionurl = "https://vsco.co/ajxp/%s/2.0/collections/%s/medias?" % (
            self.uid,
    ...
    def getJournalList(self):
        ...
        self.jour_found = self.session.get(
            self.journalurl, params={"size": 10000, "page": 1}, headers=media
        ).json()["articles"]
        self.pbarjlist = tqdm(
        ...
        for item in self.jour_found[loc]["body"]:
            if item["type"] == "image":
                ...
                self.works[loc].append(
                    [
                        "https://%s" % item["content"][0]["responsive_url"],
                        item["content"][0]["id"],
                        "img",
                    ]
                )
                ...
                self.works[loc].append(
                    [
                        "https://%s" % item["content"][0]["video_url"],
                        item["content"][0]["id"],
                        "vid",
                    ]
                )
    ...
    def makeImageList(self, num):
        ...
        num += 1
        z = self.session.get(
            self.mediaurl, params={"size": 100, "page": num}, headers=media
        ).json()["media"]
        count = len(z)
        while count > 0:
            for url in z:
